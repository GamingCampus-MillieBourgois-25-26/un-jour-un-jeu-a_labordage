...
BulletHell::Enemy::Enemy(float _fireinterval, SpawnBulletFn _spawnFn)
	: fireInterval(_fireinterval), spawnBullet(std::move(_spawnFn)) // ✅
{
}

void BulletHell::Enemy::Update(float _deltatime) {
	if (dead)return;
...
void BulletHell::Enemy::FireCircle(int _count, float _angleOffsetRad) {
	if (_count <= 0) return;
